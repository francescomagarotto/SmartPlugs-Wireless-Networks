...
public class SmartPlugsServer extends WebSocketServer {
    ...
    private void connectClient(WebSocket connection, String clientID, String clientName, String clientWatts) {
    ...
    public void onOpen(WebSocket connection, ClientHandshake handshake) {
        String clientID = handshake.getFieldValue("id");
        String clientName = handshake.getFieldValue("name");
        String clientWatts = handshake.getFieldValue("watts");

        // if there's enough wattage to connect client
        if (wattCounter.get() + Integer.parseInt(clientWatts) <= MAX_WATT_COUNTER) {
            connectClient(connection, clientID, clientName, clientWatts);
        } else {
            ...
            // if user doesn't want to disconnect other clients, telling new client it can't connect

            int sumWattage = -1;    // starting from -1 in order to avoid printing to the user that the clients chosen aren't enough
            ...
            while(sumWattage < Integer.parseInt(clientWatts)) {
                ...
                // this will be displayed only if the user tried to disconnect a few clients but weren't enough to accomodate the new client
                if (sumWattage == 0) {
            ...
            // if we are here, the user gave a correct list of devices to disconnect to accomodate the new device
            // disconnecting devices:
            for(int i=0; i<idsToDisconnect.size();i++) {
                disconnectClient(idsToDisconnect.get(i));
            }
