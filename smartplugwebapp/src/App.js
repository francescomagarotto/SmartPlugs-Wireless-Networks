...
function App() {
  ...
  const changeSwitch = (index) => {
    ...
    fetch("/plugcmd", {
      ...
      dataState.plugs[index].status = Math.abs(
        dataState.plugs[index].status - 1
      );
      setData(dataState);
    });
  };
  let value = data
    ? Math.round((data.currentConsume / data.availableWatts) * 100)
    : 0;
  return (
    <div className="App">
      ...
      <div className="row mt-4">
        <div className="col-md-4 offset-md-4">
          {data &&
            data.plugs.map((x, index) => {
              if (x.watts !== undefined)
                return (
                  <div className="card">
                    <div className="row p-2">
                      ...
                      <div className="col-4 align-self-center">
                        <Switch
                          disabled={
                            data.plugs[index].status === 0 &&
                            data.currentConsume +
                              data.plugs[index].max_power_usage >
                              data.totalWattAvailable
                          }
